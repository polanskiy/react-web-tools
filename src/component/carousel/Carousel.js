import React, {
  Children, useEffect, useState, useCallback,
} from 'react';
import './carousel.css';

const Carousel = ({
  initialSlide, children, autoPlayInterval, arrows, dots, transitionDuration, transitionTimingFunction, carouselClass, dotsBoxClass, arrowsBoxClass,
}) => {
    ...
    >
      ...
      <ol className={dotsBoxClass}>
        {Children.map(children, (item, index) => (
          <li role="presentation" aria-current={slide === index + 1} onClick={() => changeSlide(index + 1)}>
            {index + 1}
          </li>
        ))}
      </ol>
      ...
      <div className={arrowsBoxClass}>
        <button type="button" className="prevArr" onClick={goPrevSlide} />
        <button type="button" className="nextArr" onClick={goNextSlide} />
      </div>
      )}
    </div>
  );
